...
class AdminProductService {
  ...
  public async findRelatedproducts(page: string, limit: string, query,category): Promise<{ Products: Product[]; total: number; page: string }> {
    let queryData = {};
    // let queryData = { category: query.category };
    ...
    const total = await this.product.find({category:category}).countDocuments();
    return { Products, page, total };
  }
